...
with st.sidebar:
  ...
  if uploaded_files:
    ...
    def analyze_images_with_gemini(files, api_key):
      try:
        # ใช้ Client จาก SDK ตัวใหม่ (google-genai)
        ...
        # เรียกใช้โมเดล gemini-2.0-flash ที่เปิดให้ใช้ฟรีและเสถียรที่สุด
        response = client.models.generate_content(
            model="gemini-2.0-flash", contents=[prompt] + images
        )
    ...
    if st.button("🧠 ให้ Gemini วิเคราะห์รูปภาพและอัปเดตพอร์ต"):
      if not user_key:
        ...
        with st.spinner("กำลังให้ Gemini วิเคราะห์รูปภาพทั้งหมด..."):
          analysis_results = analyze_images_with_gemini(
              uploaded_files, user_key
          )
          if analysis_results:
...
with c2:
  ...
  fig_bar.update_layout(
      template="plotly_dark", yaxis_title="กำไร (%)", xaxis_title=""
  )
